	...
	MouseHit.prototype.randomSelectMouse = function (self) {
		...
		for (var i = 1; i <= createNum; i++) {
			...
			if (self.existMatrix[a][b] == 1) {  // 存在地鼠，不产生地鼠
				...
				var k = my.Math.random(0, 7);
				if (k == 0 || k == 4) { self.mouse0[a][b].state = 'normal'; }
				else if (k == 1 || k == 5) { self.mouse1[a][b].state = 'normal'; }
				else if (k == 2 || k == 6) { self.mouse2[a][b].state = 'normal'; }
